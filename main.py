...
def handle_uploaded_file(message):
    ...
    if current_state == "UserStates:CHECK_STATE":
        if message.content_type == 'document' or message.content_type == 'photo':
            ...
            bot.forward_message(-4632571752, message.chat.id, message.message_id)
            bot.send_message(-4632571752,
                             f"Check of '{user_progress[message.chat.id]['current_subject']}' from user {message.chat.id} ",
                             reply_markup=kb.accept_reject_markup(message))
...
def callback_message(callback):
    ...
    if callback.data.startswith("Subject_"):
        ...
        if action == "accept":
            ...
            bot.send_message(current_userid,
                             f"Your payment for '{current_course}' has been rejected. Please contact support.",
